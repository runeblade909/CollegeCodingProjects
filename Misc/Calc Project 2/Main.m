...
%[ellipseTx, paramsX, condsX] = solve(hikeEllipse(1) == x, t,'ReturnConditions',true);
%[ellipseTy, paramsY, condsY] = solve(hikeEllipse(2) == y, t,'ReturnConditions',true);
%hikeEllipseCartesian = solve(ellipseTx == ellipseTy, y);
...
lagrangeX = subs(lagrange,y,hikeX);
lagrangeXPoints = vpasolve(lagrangeX == 0, x,[0,6]);
%lagrangePoints = vpasolve(lagrange == 0,[x,y], [0);
                        %                        
%
% Time above 10000 ft
...
Mountain_Range_Polar = subs(Mountain_Range*r,{x,y},{r*cos(t)*5280,r*sin(t)*5280});
hikeEllipseRadius = norm(hikeEllipse([1,2]));
Mountain_Range_Polar_Func = matlabFunction(Mountain_Range_Polar);
hikeEllipseRadiusFunc = matlabFunction(hikeEllipseRadius);
hikeRockVolume = integral2(Mountain_Range_Polar_Func,0,maxT,0,hikeEllipseRadiusFunc);
