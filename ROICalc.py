...
#intake power and excavation output data

def ROI(data,MethBuckF,HydrBuckF,MethDrumF,HydrDrumF,MethConvF,HydrConvF,BuckMetal,DrumMetal,ConvMetal,scale,ProcCost,ProcTime):


    #Orbital Solar Array b = 0 
    ...
    for i in range(len(Product_Cost)):
        Product_Cost[i]=LaunchCost+LLOMass*Product_Cost[i]
    ...
    Revenue = np.zeros((2305,1))
    Value = np.zeros((6,1))
    Infrastructure_Cost = np.zeros((2305,1))
    Profit = np.zeros((2305,1))
    ...
    for k in range(0,2304):
        ...
        #print("\n\n",data["Efficiency"][k][1:-1],"\n\n")
        Revenue[index]= float(data["Efficiency"][k][1:-1])*float(data["Lifetime"][k][1:-1])*870*TotalVal   ###float(data["Lifetime"][:])*(870)*TotalVal
        index = index + 1




        #Infrastructure_Cost[index] = float(Ground_LS[i])*float(Ground_Eff[i][k])*870*(float(Ground_Cost[i])+float(Processing_Cost[k][i])+float(Transportation_Cost[h]))+float(Power_LS[j])*float(Power_Eff[j])*31536000*float(Power_Cost[j])
        Infrastructure_Cost[index] = float(data["Cost"][k][1:-1])
        Profit[index] = Revenue[index]-Infrastructure_Cost[index]+3000000000;
    #print("Revenue")
    #print(Revenue)
