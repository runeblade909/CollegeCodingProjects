...
{
    free(currNode); // deletes node
    return NULL;    // returns empty pointer
}
...
{
    free(currNode); // deletes node
    return currNode->right; // returns pointer to right child
}
...
{
    free(currNode); // deletes node
    return currNode->left; //  returns left child
}
...
Node *cur = getMinValueNode(currNode->right); // finds the minumum value in the right subtree

currNode->right = deleteNode(currNode->right, cur->key); // replace the node with everything from that minumum and its new sub tree
...
// GOLD TODO
bool BST::isValidBSTUtil(Node *root, int &prev){
...
bool BST::isValidBST(){
    ...
    int prev = getMinValueNode(root)->key;
