...
void Graph::addEdge(int v1, int v2){
    ...
    for(int i = 0; i < vertices.size(); i++){
        if(vertices[i]->key == v1){         // Look through the vertices vector for first item
            for(int j = 0; j < vertices.size(); j++){ 
                if(vertices[j]->key == v2 && i != j){ // Look through vertices vector for second time
                    adjVertex av;        //adjecent items  
...
void Graph::removeEdge(int key1, int key2)
{
  ...
  // Ex: say you need to remove ith index item from a vector "myvector", you can use myvector.erase(myvector.begin()
...
bool Graph::isBridge(int key1, int key2)
{
  ...
  // Step 4: Add the edge back to the graph
  addEdge(key1, key2);
