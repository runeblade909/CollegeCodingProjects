...
[pos,vel_out,g_forces] = loop_xz(xyz,v_in,5,.9);
...
dz =  z- zin;                                        %difference in height from start to end of the loop
mag_v_in = sqrt(vx.^2+vz.^2);

speed =  mag_v_in - sign(dz).*sqrt(abs(2*g.*(dz)));                                 %speed coming out of the loop
vel = [-(speed.*cos(theta))' (0.*cos(theta))' (speed.*sin(theta + (pi/2)))'];
...
g_forces = [(gs(:,1).*cos(theta))' (0.*cos(theta))' (gs(:,3).*sin(theta))'];   % G-force matrix [front/back, left/right, up/down]
